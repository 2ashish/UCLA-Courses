...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dims=100, num_classes=10,
    ...
    self.params['W1'] = np.random.normal(loc=0.0,scale=weight_scale, size = (self.input_dim,self.hidden_dims))
    self.params['b1'] = np.zeros((self.hidden_dims))
    self.params['W2'] = np.random.normal(loc=0.0,scale=weight_scale, size = (self.hidden_dims,self.num_classes))
  ...
  def loss(self, X, y=None):
    ...
    scores, cache_2 = affine_forward(layer_1,self.params['W2'],self.params['b2'])
    ...
    loss, d2 = softmax_loss(scores,y) 
    loss+= 0.5*self.reg*(np.sum(np.square(self.params['W1'])) + np.sum(np.square(self.params['W2'])))
    d1,dw2,db2 = affine_backward(d2,cache_2)
    dx,dw1,db1 = affine_relu_backward(d1,cache_1)
  ...
  def loss(self, X, y=None):
    ...
    for i in np.arange(self.num_layers-1,-1,-1):
        ...
        if i==self.num_layers-1:
            da,grads[w], grads[b] = affine_backward(ds,cache[i])
        else:
            da,grads[w], grads[b] = affine_relu_backward(da,cache[i])
