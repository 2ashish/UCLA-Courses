...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    layer_1 = X.dot(W1.T)+b1
    relu_1 = np.maximum(layer_1,0)
    layer_2 = relu_1.dot(W2.T)+b2
    scores = layer_2
    ...
    scores = scores.T
    ma_scores = scores.max(axis=0, keepdims=True)
    scores -= ma_scores
    exp_scores = np.exp(scores) #(c,n)
    scores_sum = np.sum(exp_scores,axis=0) #(1,n)
    loglike = np.log(np.squeeze(np.take_along_axis(exp_scores,y[np.newaxis,:],0),0)/scores_sum)
    softmax_loss = -np.average(loglike)
    
    l2_reg_loss = 0.5*(np.sum(np.square(W1)) + np.sum(np.square(W2)))
    loss = softmax_loss + reg*l2_reg_loss
    ...
    scores_temp = (exp_scores/scores_sum)
    scores_temp[y, np.arange(X.shape[0])] -= 1
    scores_temp/= X.shape[0]
    grads['W2'] = (scores_temp).dot(relu_1)
    grads['b2'] = np.sum(scores_temp, axis=1)
    
    dld_relu = scores_temp.T.dot(W2)
    dld_relu[layer_1<0] = 0
    grads['b1'] = np.sum(dld_relu,axis=0)
    grads['W1'] = dld_relu.T.dot(X)
