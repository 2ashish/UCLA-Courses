...
void write_block_bitmap(int fd) {
	...
	for(int i=NUM_BLOCKS;i<=8*BLOCK_SIZE;i++){
		buf[(i-1)/8] |= 1<<((i-1)%8);
	}
...
void write_inode_table(int fd) {
	...
	struct ext2_inode hello2_inode = {0};
	hello2_inode.i_mode = EXT2_S_IFLNK
	                              | EXT2_S_IRUSR
	                              | EXT2_S_IWUSR
	                              | EXT2_S_IRGRP
	                              | EXT2_S_IROTH;
	hello2_inode.i_uid = 1000;
	hello2_inode.i_size = 11;
	hello2_inode.i_atime = current_time;
	hello2_inode.i_ctime = current_time;
	hello2_inode.i_mtime = current_time;
	hello2_inode.i_dtime = 0;
	hello2_inode.i_gid = 1000;
	hello2_inode.i_links_count = 1;
	hello2_inode.i_blocks = 0; /* These are oddly 512 blocks */
	char *buf = "hello-world";                                                 
	size_t len = strlen(buf); 
	memcpy(&hello2_inode.i_block, buf, len);
	write_inode(fd, HELLO_INO, &hello2_inode);
...
void write_root_dir_block(int fd) {
	...
	struct ext2_dir_entry child2_entry = {0};
	dir_entry_set(child2_entry, HELLO_WORLD_INO, "hello-world");
	dir_entry_write(child2_entry, fd);

	bytes_remaining -= child2_entry.rec_len;

	struct ext2_dir_entry child3_entry = {0};
	dir_entry_set(child3_entry, HELLO_INO, "hello");
	dir_entry_write(child3_entry, fd);

	bytes_remaining -= child3_entry.rec_len;
